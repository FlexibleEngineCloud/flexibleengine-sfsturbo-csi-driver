...
// NewHwEFSV1
func NewHwEFSV1(client *golangsdk.ProviderClient, eo golangsdk.EndpointOpts) (*golangsdk.ServiceClient, error) {
	klog.V(4).Infof("efs client begin")
	sc, err := initClientOpts(client, eo, "network")
	sc.Endpoint = strings.Replace(sc.Endpoint, "vpc", "sfs-turbo", 1)
	sc.ResourceBase = sc.Endpoint + "v1/" + client.ProjectID + "/"
	klog.V(4).Infof("efs client base: %v", sc.ResourceBase)
	return sc, err
}
